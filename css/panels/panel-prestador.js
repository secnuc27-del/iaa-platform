...
const PanelPrestador = (function () {
  ...
  const NAV_ITEMS = [
    ...
    { id: 'profile',       icon: '👤', label: 'Meu Perfil Pro' },
  ...
  function renderSidebar() {
    const sidebar = document.getElementById('sidebar-prestador');
    if (!sidebar) return;
    sidebar.innerHTML = `
      ...
      <nav class="sidebar-nav">
        ${NAV_ITEMS.map(item => `
          <div class="nav-item${_currentSection === item.id ? ' active' : ''}"
               ...
               onclick="PanelPrestador.navigate('${item.id}')">
  ...
  function renderTopbar() {
    const topbar = document.getElementById('topbar-prestador');
    if (!topbar) return;
    ...
    topbar.innerHTML = `
      <button class="icon-btn" onclick="toggleSidebarPanel('sidebar-prestador')" title="Menu">☰</button>
      ...
      <div class="topbar-right">
        ...
        <div class="topbar-avatar" id="topbar-avatar"
             onclick="PanelPrestador.navigate('profile')"
             style="cursor:pointer;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:var(--accent);color:#fff;font-weight:700;font-size:14px;overflow:hidden">
          ${avatarContent}
        </div>
      </div>`;
  }
  ...
  function updateActiveNav() {
    document.querySelectorAll('#sidebar-prestador .nav-item[data-section]').forEach(el => {
    ...
    navigate(section) {
      ...
      if (typeof renders !== 'undefined' && renders[_currentSection]) {
        Promise.resolve(renders[_currentSection](content)).catch(err => {
          ...
          if (content) { var _sec = _currentSection; content.innerHTML = '<div style="padding:48px;text-align:center;color:var(--text2)"><div style="font-size:40px;margin-bottom:12px">⚠️</div><p>Erro ao carregar. Verifique sua conexão.</p><button class="btn btn-outline btn-sm" style="margin-top:12px" onclick="PanelPrestador.navigate(this.dataset.sec)" data-sec="' + _sec + '">Tentar novamente</button></div>'; }
        });
      } else {
